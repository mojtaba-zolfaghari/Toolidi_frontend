...
export class AdminAgentsComponent implements OnInit {
  ...
  openEdit(agent: AdminAgent): void { this.editingAgent = agent; this.agentForm.patchValue(agent); this.formError = ''; this.formOpen = true; const province = this.provinces.find(p => p.name === agent.province); if (province) { this.agentForm.patchValue({ provinceId: province.id }); this.onProvinceChange(); setTimeout(() => { const city = this.cities.find(c => c.name === agent.city); if (city) this.agentForm.patchValue({ cityId: city.id }); }, 0); } }
